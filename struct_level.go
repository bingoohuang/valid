...
// StructLevel contains all the information and helper functions to valid a struct.
type StructLevel interface {

	// returns the main validation object, in case one wants to call validations internally.
	// this is so you don't have to use anonymous functions to get access to the valid
	// instance.
	Validator() *Validate
	...
	// on the nesting. most of the time they will be blank, unless you valid
	// at a level lower the the current field depth
	ReportValidationErrors(relativeNamespace, relativeActualNamespace string, errs ValidationErrors)
}

var _ StructLevel = new(validator)
...
func (v *validator) Top() reflect.Value { return v.top }
...
// this is only called when within Struct and Field Level validation and
// should not be relied upon for an acurate value otherwise.
func (v *validator) Parent() reflect.Value { return v.slflParent }

// Current returns the current struct.
func (v *validator) Current() reflect.Value { return v.slCurrent }

// Validator returns the main validation object, in case one want to call validations internally.
func (v *validator) Validator() *Validate { return v.v }

// ExtractType gets the actual underlying type of field value.
func (v *validator) ExtractType(field reflect.Value) (reflect.Value, reflect.Kind, bool) {
	return v.extractTypeInternal(field, false)
}

// ReportError reports an error just by passing the field and tag information.
func (v *validator) ReportError(field interface{}, fieldName, structFieldName, tag, param string) {
...
func (v *validator) ReportValidationErrors(relativeNamespace, relativeStructNamespace string, errs ValidationErrors) {
	var err *fieldError
