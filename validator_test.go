...
// ./valid.test -test.bench=. -test.cpuprofile=cpu.prof
// go tool pprof valid.test cpu.prof
//
//
...
type TestString struct {
	...
	Lt        string `valid:"lt=10"`
	Lte       string `valid:"lte=10"`
	Gt        string `valid:"gt=10"`
	Gte       string `valid:"gte=10"`
	OmitEmpty string `valid:"omitempty,min=1,max=10"`
	Sub       *SubTest
...
type TestUint64 struct {
	Required  uint64 `valid:"required"`
	Len       uint64 `valid:"len=10"`
	Min       uint64 `valid:"min=1"`
	Max       uint64 `valid:"max=10"`
	MinMax    uint64 `valid:"min=1,max=10"`
	OmitEmpty uint64 `valid:"omitempty,min=1,max=10"`
}

type TestFloat64 struct {
	Required  float64 `valid:"required"`
	Len       float64 `valid:"len=10"`
	Min       float64 `valid:"min=1"`
	Max       float64 `valid:"max=10"`
	MinMax    float64 `valid:"min=1,max=10"`
	Lte       float64 `valid:"lte=10"`
	OmitEmpty float64 `valid:"omitempty,min=1,max=10"`
}

type TestSlice struct {
	Required  []int `valid:"required"`
	Len       []int `valid:"len=10"`
	Min       []int `valid:"min=1"`
	Max       []int `valid:"max=10"`
	MinMax    []int `valid:"min=1,max=10"`
	OmitEmpty []int `valid:"omitempty,min=1,max=10"`
}
...
type CustomMadeUpStruct struct {
	MadeUp        MadeUpCustomType `valid:"required"`
	OverriddenInt int              `valid:"gt=1"`
}
...
type TestPartial struct {
	...
	BlankTag  string     `valid:""`
	Required  string     `valid:"required"`
	SubSlice  []*SubTest `valid:"required,dive"`
	Sub       *SubTest
	SubIgnore *SubTest `valid:"-"`
	Anonymous struct {
		A         string     `valid:"required"`
		ASubSlice []*SubTest `valid:"required,dive"`

		SubAnonStruct []struct {
			Test      string `valid:"required"`
			OtherTest string `valid:"required"`
		} `valid:"required,dive"`
	}
}

type TestStruct struct {
	String string `valid:"required" json:"StringVal"`
}
...
type TestStructReturnValidationErrorsInner2 struct {
	String string `valid:"required" json:"JSONString"`
}
...
func TestStructLevelInvalidError(t *testing.T) {
	valid := New()
	valid.RegisterStructValidation(StructLevelInvalidError, StructLevelInvalidErr{})

	var test StructLevelInvalidErr

	err := valid.Struct(test)
	NotEqual(t, err, nil)
...
func TestNameNamespace(t *testing.T) {
	type Inner2Namespace struct {
		String []string `valid:"dive,required" json:"JSONString"`
	}
...
func TestReg(t *testing.T) {
	type Test struct {
		A string `valid:"reg=a"`
	}

	va := New()
	Equal(t, va.RegisterReg("a", `\w{3,}`), nil)
	err := va.Struct(Test{A: "abcd"})
	Equal(t, err, nil)

	err = va.Struct(Test{A: "ab"})
	NotEqual(t, err, nil)
	//fmt.Printf("%v", err)
	errs := err.(ValidationErrors)

	Equal(t, len(errs), 1)
	AssertError(t, errs, "Test.A", "Test.A", "A", "A", "reg")

	type Test2 struct {
		A string `valid:"reg=a+"`
	}

	err = va.Struct(Test2{A: "aaaa"})
	Equal(t, err, nil)

	err = va.Struct(Test2{A: "bbb"})
	NotEqual(t, err, nil)
	//fmt.Printf("%v", err)
	errs = err.(ValidationErrors)

	Equal(t, len(errs), 1)
	AssertError(t, errs, "Test2.A", "Test2.A", "A", "A", "reg")
}
...
func TestAnonymous(t *testing.T) {
	...
	type Test struct {
		Anonymous struct {
			A string `valid:"required" json:"EH"`
		}
		AnonymousB struct {
			B string `valid:"required" json:"BEE"`
		}
		anonymousC struct {
...
func TestAnonymousSameStructDifferentTags(t *testing.T) {
	...
	tst = &Test{
		A: struct {
			A string `valid:"omitempty,required"`
		}{
			A: "",
		},
	}
...
func TestStructLevelReturnValidationErrors(t *testing.T) {
	valid := New()
	valid.RegisterStructValidation(StructValidationTestStructReturnValidationErrors, TestStructReturnValidationErrors{})
...
func TestStructLevelReturnValidationErrorsWithJSON(t *testing.T) {
	...
	valid.RegisterStructValidation(StructValidationTestStructReturnValidationErrors2, TestStructReturnValidationErrors{})
...
func TestAliasTags(t *testing.T) {
	valid := New()
	valid.RegisterAlias("iscoloralias", "hexcolor|rgb|rgba|hsl|hsla")

	s := "rgb(255,255,255)"
	errs := valid.Var(s, "iscoloralias")
	Equal(t, errs, nil)

	s = ""
	errs = valid.Var(s, "omitempty,iscoloralias")
	Equal(t, errs, nil)

	s = "rgb(255,255,0)"
	errs = valid.Var(s, "iscoloralias,len=5")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "len")

	type Test struct {
		Color string `valid:"iscoloralias"`
	}
	...
	valid.RegisterAlias("req", "required,dive,iscoloralias")
	arr := []string{"val1", "#fff", "#000"}

	errs = valid.Var(arr, "req")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "[0]", "[0]", "[0]", "[0]", "iscoloralias")

	PanicMatches(t, func() { valid.RegisterAlias("exists!", "gt=5,lt=10") }, "Alias 'exists!' either contains restricted characters or is the same as a restricted tag needed for normal operation")
}
...
func TestStructPartial(t *testing.T) {
	...
	// the default state
	errs := valid.StructPartialCtx(context.Background(), tPartial, p1...)
	Equal(t, errs, nil)

	errs = valid.StructPartial(tPartial, p2...)
	Equal(t, errs, nil)

	// this isn't really a robust test, but is ment to illustrate the ANON CASE below
	errs = valid.StructPartial(tPartial.SubSlice[0], p3...)
	Equal(t, errs, nil)

	errs = valid.StructExceptCtx(context.Background(), tPartial, p1...)
	Equal(t, errs, nil)

	errs = valid.StructExcept(tPartial, p2...)
	Equal(t, errs, nil)

	// mod tParial for required feild and re-test making sure invalid fields are NOT required:
	tPartial.Required = ""

	errs = valid.StructExcept(tPartial, p1...)
	Equal(t, errs, nil)
	...
	errs = valid.StructPartial(tPartial, p1...)
	NotEqual(t, errs, nil)
	...
	errs = valid.StructExcept(tPartial.Anonymous, p4...)
	Equal(t, errs, nil)
...
func TestCrossStructLteFieldValidation(t *testing.T) {
	...
	type Test struct {
		Inner     *Inner
		CreatedAt *time.Time `valid:"ltecsfield=Inner.CreatedAt"`
		String    string     `valid:"ltecsfield=Inner.String"`
		Int       int        `valid:"ltecsfield=Inner.Int"`
		Uint      uint       `valid:"ltecsfield=Inner.Uint"`
		Float     float64    `valid:"ltecsfield=Inner.Float"`
		Array     []string   `valid:"ltecsfield=Inner.Array"`
	}
	...
	errs = valid.VarWithValueCtx(context.Background(), 1, "", "ltecsfield")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "ltecsfield")

	// this test is for the WARNING about unforeseen validation issues.
	errs = valid.VarWithValue(test, now, "ltecsfield")
	NotEqual(t, errs, nil)
	...
	type Test2 struct {
		Value Other
		Time  time.Time `valid:"ltecsfield=Value"`
	}
...
func TestCrossStructLtFieldValidation(t *testing.T) {
	...
	type Test struct {
		Inner     *Inner
		CreatedAt *time.Time `valid:"ltcsfield=Inner.CreatedAt"`
		String    string     `valid:"ltcsfield=Inner.String"`
		Int       int        `valid:"ltcsfield=Inner.Int"`
		Uint      uint       `valid:"ltcsfield=Inner.Uint"`
		Float     float64    `valid:"ltcsfield=Inner.Float"`
		Array     []string   `valid:"ltcsfield=Inner.Array"`
	}
	...
	errs = valid.VarWithValue(1, "", "ltcsfield")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "ltcsfield")

	// this test is for the WARNING about unforeseen validation issues.
	errs = valid.VarWithValue(test, now, "ltcsfield")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "Test.CreatedAt", "Test.CreatedAt", "CreatedAt", "CreatedAt", "ltcsfield")
	AssertError(t, errs, "Test.String", "Test.String", "String", "String", "ltcsfield")
	AssertError(t, errs, "Test.Int", "Test.Int", "Int", "Int", "ltcsfield")
	...
	type Test2 struct {
		Value Other
		Time  time.Time `valid:"ltcsfield=Value"`
	}
...
func TestCrossStructGteFieldValidation(t *testing.T) {
	...
	type Test struct {
		Inner     *Inner
		CreatedAt *time.Time `valid:"gtecsfield=Inner.CreatedAt"`
		String    string     `valid:"gtecsfield=Inner.String"`
		Int       int        `valid:"gtecsfield=Inner.Int"`
		Uint      uint       `valid:"gtecsfield=Inner.Uint"`
		Float     float64    `valid:"gtecsfield=Inner.Float"`
		Array     []string   `valid:"gtecsfield=Inner.Array"`
	}
	...
	errs = valid.VarWithValue(1, "", "gtecsfield")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "gtecsfield")

	// this test is for the WARNING about unforeseen validation issues.
	errs = valid.VarWithValue(test, now, "gtecsfield")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "Test.CreatedAt", "Test.CreatedAt", "CreatedAt", "CreatedAt", "gtecsfield")
	AssertError(t, errs, "Test.String", "Test.String", "String", "String", "gtecsfield")
	AssertError(t, errs, "Test.Int", "Test.Int", "Int", "Int", "gtecsfield")
	...
	type Test2 struct {
		Value Other
		Time  time.Time `valid:"gtecsfield=Value"`
	}
...
func TestCrossStructGtFieldValidation(t *testing.T) {
	...
	type Test struct {
		Inner     *Inner
		CreatedAt *time.Time `valid:"gtcsfield=Inner.CreatedAt"`
		String    string     `valid:"gtcsfield=Inner.String"`
		Int       int        `valid:"gtcsfield=Inner.Int"`
		Uint      uint       `valid:"gtcsfield=Inner.Uint"`
		Float     float64    `valid:"gtcsfield=Inner.Float"`
		Array     []string   `valid:"gtcsfield=Inner.Array"`
	}
	...
	errs = valid.VarWithValue(1, "", "gtcsfield")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "gtcsfield")

	// this test is for the WARNING about unforeseen validation issues.
	errs = valid.VarWithValue(test, now, "gtcsfield")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "Test.CreatedAt", "Test.CreatedAt", "CreatedAt", "CreatedAt", "gtcsfield")
	AssertError(t, errs, "Test.String", "Test.String", "String", "String", "gtcsfield")
	AssertError(t, errs, "Test.Int", "Test.Int", "Int", "Int", "gtcsfield")
	...
	type Test2 struct {
		Value Other
		Time  time.Time `valid:"gtcsfield=Value"`
	}
...
func TestCrossStructNeFieldValidation(t *testing.T) {
	...
	type Test struct {
		Inner     *Inner
		CreatedAt *time.Time `valid:"necsfield=Inner.CreatedAt"`
	}
	...
	errs = valid.VarWithValue(s, s2, "necsfield")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "necsfield")

	errs = valid.VarWithValue(i2, i, "necsfield")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "necsfield")

	errs = valid.VarWithValue(j2, j, "necsfield")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "necsfield")

	errs = valid.VarWithValue(k2, k, "necsfield")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "necsfield")

	errs = valid.VarWithValue(arr2, arr, "necsfield")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "necsfield")

	errs = valid.VarWithValue(now2, now, "necsfield")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "necsfield")

	errs = valid.VarWithValue(arr3, arr, "necsfield")
	Equal(t, errs, nil)
	...
	type TStruct struct {
		Inner     *SInner
		CreatedAt *time.Time `valid:"necsfield=Inner"`
	}
	...
	errs = valid.VarWithValue(nil, 1, "necsfield")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "necsfield")
}
...
func TestCrossStructEqFieldValidation(t *testing.T) {
	...
	type Test struct {
		Inner     *Inner
		CreatedAt *time.Time `valid:"eqcsfield=Inner.CreatedAt"`
	}
	...
	errs = valid.VarWithValue(s, s2, "eqcsfield")
	Equal(t, errs, nil)

	errs = valid.VarWithValue(i2, i, "eqcsfield")
	Equal(t, errs, nil)

	errs = valid.VarWithValue(j2, j, "eqcsfield")
	Equal(t, errs, nil)

	errs = valid.VarWithValue(k2, k, "eqcsfield")
	Equal(t, errs, nil)

	errs = valid.VarWithValue(arr2, arr, "eqcsfield")
	Equal(t, errs, nil)

	errs = valid.VarWithValue(now2, now, "eqcsfield")
	Equal(t, errs, nil)

	errs = valid.VarWithValue(arr3, arr, "eqcsfield")
	NotEqual(t, errs, nil)
	...
	type TStruct struct {
		Inner     *SInner
		CreatedAt *time.Time `valid:"eqcsfield=Inner"`
	}
	...
	errs = valid.VarWithValue(nil, 1, "eqcsfield")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "eqcsfield")
}
...
func TestCrossNamespaceFieldValidation(t *testing.T) {
	...
	Equal(t, current.String(), "<*valid.SliceStruct Value>")
...
func TestExistsValidation(t *testing.T) {
	...
	type Thing struct {
		Truthiness *bool `json:"truthiness" valid:"required"`
	}
...
func TestSQLValue2Validation(t *testing.T) {
	valid := New()
	valid.RegisterCustomTypeFunc(ValidateValuerType, valuer{}, (*driver.Valuer)(nil), sql.NullString{}, sql.NullInt64{}, sql.NullBool{}, sql.NullFloat64{})
	valid.RegisterCustomTypeFunc(ValidateCustomType, MadeUpCustomType{})
	valid.RegisterCustomTypeFunc(OverrideIntTypeForSomeReason, 1)

	val := valuer{
		Name: "",
	}

	errs := valid.Var(val, "required")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "required")

	val.Name = "Valid Name"
	errs = valid.VarCtx(context.Background(), val, "required")
	Equal(t, errs, nil)

	val.Name = "errorme"

	PanicMatches(t, func() { _ = valid.Var(val, "required") }, "SQL Driver Valuer error: some kind of error")

	myVal := valuer{
		Name: "",
	}

	errs = valid.Var(myVal, "required")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "required")
...
func TestSQLValueValidation(t *testing.T) {
	valid := New()
	valid.RegisterCustomTypeFunc(ValidateValuerType, (*driver.Valuer)(nil), valuer{})
	valid.RegisterCustomTypeFunc(ValidateCustomType, MadeUpCustomType{})
	valid.RegisterCustomTypeFunc(OverrideIntTypeForSomeReason, 1)
	...
	PanicMatches(t, func() { errs = valid.Var(val, "required") }, "SQL Driver Valuer error: some kind of error")
...
func TestIPv6Validation(t *testing.T) {
	...
	for i, test := range tests {
		...
		errs := valid.Var(test.param, "ipv6")
...
func TestIPv4Validation(t *testing.T) {
	...
	for i, test := range tests {
		...
		errs := valid.Var(test.param, "ipv4")
...
func TestCIDRValidation(t *testing.T) {
	...
	for i, test := range tests {
		...
		errs := valid.Var(test.param, "cidr")
...
func TestCIDRv6Validation(t *testing.T) {
	...
	for i, test := range tests {
		...
		errs := valid.Var(test.param, "cidrv6")
...
func TestCIDRv4Validation(t *testing.T) {
	...
	for i, test := range tests {
		...
		errs := valid.Var(test.param, "cidrv4")
...
func TestTCPAddrValidation(t *testing.T) {
	...
	for i, test := range tests {
		errs := valid.Var(test.param, "tcp_addr")
		if test.expected {
			if !IsEqual(errs, nil) {
...
func TestTCP6AddrValidation(t *testing.T) {
	...
	for i, test := range tests {
		errs := valid.Var(test.param, "tcp6_addr")
		if test.expected {
			if !IsEqual(errs, nil) {
...
func TestTCP4AddrValidation(t *testing.T) {
	...
	for i, test := range tests {
		errs := valid.Var(test.param, "tcp4_addr")
		if test.expected {
			if !IsEqual(errs, nil) {
...
func TestUDPAddrValidation(t *testing.T) {
	...
	for i, test := range tests {
		errs := valid.Var(test.param, "udp_addr")
		if test.expected {
			if !IsEqual(errs, nil) {
...
func TestUDP6AddrValidation(t *testing.T) {
	...
	for i, test := range tests {
		errs := valid.Var(test.param, "udp6_addr")
		if test.expected {
			if !IsEqual(errs, nil) {
...
func TestUDP4AddrValidation(t *testing.T) {
	...
	for i, test := range tests {
		errs := valid.Var(test.param, "udp4_addr")
		if test.expected {
			if !IsEqual(errs, nil) {
...
func TestIPAddrValidation(t *testing.T) {
	...
	for i, test := range tests {
		errs := valid.Var(test.param, "ip_addr")
		if test.expected {
			if !IsEqual(errs, nil) {
...
func TestIP6AddrValidation(t *testing.T) {
	...
	for i, test := range tests {
		errs := valid.Var(test.param, "ip6_addr")
		if test.expected {
			if !IsEqual(errs, nil) {
...
func TestIP4AddrValidation(t *testing.T) {
	...
	for i, test := range tests {
		errs := valid.Var(test.param, "ip4_addr")
		if test.expected {
			if !IsEqual(errs, nil) {
...
func TestUnixAddrValidation(t *testing.T) {
	...
	for i, test := range tests {
		errs := valid.Var(test.param, "unix_addr")
		if test.expected {
			if !IsEqual(errs, nil) {
...
func TestSliceMapArrayChanFuncPtrInterfaceRequiredValidation(t *testing.T) {
	...
	errs = valid.Var(iface, "required")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "required")

	errs = valid.Var(iface, "omitempty,required")
	Equal(t, errs, nil)

	errs = valid.Var(iface, "")
	Equal(t, errs, nil)

	errs = valid.VarWithValue(nil, iface, "")
	Equal(t, errs, nil)

	var f func(string)

	errs = valid.Var(f, "required")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "required")
...
func TestCommaAndPipeObfuscationValidation(t *testing.T) {
	...
	errs := valid.Var(s, "excludesall=0x2C")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "excludesall")

	errs = valid.Var(s, "excludesall=0x7C")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "excludesall")
}
...
func TestBadKeyValidation(t *testing.T) {
	...
	PanicMatches(t, func() { _ = valid.Struct(tst) }, "Undefined validation function ' ' on field 'Name'")

	type Test2 struct {
		Name string `valid:"required,,len=2"`
	}
	...
	PanicMatches(t, func() { _ = valid.Struct(tst2) }, "Invalid validation tag on field 'Name'")
}
...
func TestInterfaceErrValidation(t *testing.T) {
	...
	valid := New()
	errs := valid.Var(v1, "len=1")
	Equal(t, errs, nil)

	errs = valid.Var(v2, "len=1")
	Equal(t, errs, nil)

	type ExternalCMD struct {
		Userid string      `json:"userid"`
		Action uint32      `json:"action"`
		Data   interface{} `json:"data,omitempty" valid:"required"`
	}
	...
	type ExternalCMD2 struct {
		Userid string      `json:"userid"`
		Action uint32      `json:"action"`
		Data   interface{} `json:"data,omitempty" valid:"len=1"`
	}
	...
	type TestMultiDimensionalStructs struct {
		Errs [][]interface{} `valid:"gt=0,dive,dive"`
	}
	...
	type TestMultiDimensionalStructsPtr2 struct {
		Errs [][]*Inner `valid:"gt=0,dive,dive,required"`
	}
	...
	errs = valid.Var(m, "len=3,dive,len=2")
	NotEqual(t, errs, nil)
	Equal(t, len(errs.(ValidationErrors)), 1)
	AssertError(t, errs, "[3]", "[3]", "[3]", "[3]", "len")

	errs = valid.Var(m, "len=2,dive,required")
	NotEqual(t, errs, nil)
	Equal(t, len(errs.(ValidationErrors)), 1)
	AssertError(t, errs, "", "", "", "", "len")
	...
	errs = valid.Var(arr, "len=3,dive,len=2")
	NotEqual(t, errs, nil)
	Equal(t, len(errs.(ValidationErrors)), 1)
	AssertError(t, errs, "[1]", "[1]", "[1]", "[1]", "len")

	errs = valid.Var(arr, "len=2,dive,required")
	NotEqual(t, errs, nil)
	Equal(t, len(errs.(ValidationErrors)), 1)
	AssertError(t, errs, "", "", "", "", "len")
...
func TestMapDiveValidation(t *testing.T) {
	...
	errs := valid.Var(n, "omitempty,required")
	Equal(t, errs, nil)

	m := map[int]string{0: "ok", 3: "", 4: "ok"}

	errs = valid.Var(m, "len=3,dive,required")
	NotEqual(t, errs, nil)
	Equal(t, len(errs.(ValidationErrors)), 1)
	...
	type TestMapStruct struct {
		Errs map[int]Inner `valid:"gt=0,dive"`
	}
	...
	type TestMapTimeStruct struct {
		Errs map[int]*time.Time `valid:"gt=0,dive,required"`
	}
	...
	type TestMapStructPtr struct {
		Errs map[int]*Inner `valid:"gt=0,dive,required"`
	}
	...
	type TestMapStructPtr2 struct {
		Errs map[int]*Inner `valid:"gt=0,dive,omitempty,required"`
	}
	...
	type MapDiveJSONTest struct {
		Map map[string]string `valid:"required,gte=1,dive,gte=1" json:"MyName"`
	}
...
func TestArrayDiveValidation(t *testing.T) {
	...
	errs := valid.Var(arr, "len=3,dive,required")
	NotEqual(t, errs, nil)
	Equal(t, len(errs.(ValidationErrors)), 1)
	...
	PanicMatches(t, func() { _ = valid.Struct(bd) }, "dive error! can't dive on a non slice or map")

	type Test struct {
		Errs []string `valid:"gt=0,dive,required"`
	}
	...
	type TestMultiDimensional struct {
		Errs [][]string `valid:"gt=0,dive,dive,required"`
	}
	...
	type TestMultiDimensionalStructs struct {
		Errs [][]Inner `valid:"gt=0,dive,dive"`
	}
	...
	type TestMultiDimensionalStructsPtr3 struct {
		Errs [][]*Inner `valid:"gt=0,dive,dive,omitempty"`
	}
	...
	type TestMultiDimensionalTimeTime struct {
		Errs [][]*time.Time `valid:"gt=0,dive,dive,required"`
	}
...
func TestLongitudeValidation(t *testing.T) {
	...
	for i, test := range tests {
		...
		errs := valid.Var(test.param, "longitude")
	...
	PanicMatches(t, func() { _ = valid.Var(true, "longitude") }, "Bad field type bool")
}
...
func TestLatitudeValidation(t *testing.T) {
	...
	for i, test := range tests {
		...
		errs := valid.Var(test.param, "latitude")
	...
	PanicMatches(t, func() { _ = valid.Var(true, "latitude") }, "Bad field type bool")
}
...
func TestDataURIValidation(t *testing.T) {
	...
	for i, test := range tests {
		errs := valid.Var(test.param, "datauri")
...
func TestMultibyteValidation(t *testing.T) {
	...
	for i, test := range tests {
		...
		errs := valid.Var(test.param, "multibyte")
...
func TestPrintableASCIIValidation(t *testing.T) {
	...
	for i, test := range tests {
		...
		errs := valid.Var(test.param, "printascii")
...
func TestASCIIValidation(t *testing.T) {
	...
	for i, test := range tests {
		...
		errs := valid.Var(test.param, "ascii")
...
func TestUUID5Validation(t *testing.T) {
	...
	for i, test := range tests {
		...
		errs := valid.Var(test.param, "uuid5")
...
func TestUUID4Validation(t *testing.T) {
	...
	for i, test := range tests {
		...
		errs := valid.Var(test.param, "uuid4")
...
func TestUUID3Validation(t *testing.T) {
	...
	for i, test := range tests {
		...
		errs := valid.Var(test.param, "uuid3")
...
func TestUUIDValidation(t *testing.T) {
	...
	for i, test := range tests {
		...
		errs := valid.Var(test.param, "uuid")
...
func TestUUID5RFC4122Validation(t *testing.T) {
	...
	for i, test := range tests {
		...
		errs := valid.Var(test.param, "uuid5_rfc4122")
...
func TestUUID4RFC4122Validation(t *testing.T) {
	...
	for i, test := range tests {
		...
		errs := valid.Var(test.param, "uuid4_rfc4122")
...
func TestUUID3RFC4122Validation(t *testing.T) {
	...
	for i, test := range tests {
		...
		errs := valid.Var(test.param, "uuid3_rfc4122")
...
func TestUUIDRFC4122Validation(t *testing.T) {
	...
	for i, test := range tests {
		...
		errs := valid.Var(test.param, "uuid_rfc4122")
...
func TestISBNValidation(t *testing.T) {
	...
	for i, test := range tests {
		...
		errs := valid.Var(test.param, "isbn")
...
func TestISBN13Validation(t *testing.T) {
	...
	for i, test := range tests {
		...
		errs := valid.Var(test.param, "isbn13")
...
func TestISBN10Validation(t *testing.T) {
	...
	for i, test := range tests {
		...
		errs := valid.Var(test.param, "isbn10")
...
func TestExcludesRuneValidation(t *testing.T) {
	tests := []struct {
		Value       string `valid:"excludesrune=☻"`
		Tag         string
		ExpectedNil bool
	}{
...
func TestExcludesAllValidation(t *testing.T) {
	tests := []struct {
		Value       string `valid:"excludesall=@!{}[]"`
		Tag         string
		ExpectedNil bool
	}{
	...
	errs := valid.Var(username, "excludesall=@ ")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "excludesall")

	excluded := ","

	errs = valid.Var(excluded, "excludesall=!@#$%^&*()_+.0x2C?")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "excludesall")
...
func TestContainsRuneValidation(t *testing.T) {
	tests := []struct {
		Value       string `valid:"containsrune=☻"`
		Tag         string
		ExpectedNil bool
	}{
...
func TestContainsAnyValidation(t *testing.T) {
	tests := []struct {
		Value       string `valid:"containsany=@!{}[]"`
		Tag         string
		ExpectedNil bool
	}{
...
func TestIsNeFieldValidation(t *testing.T) {
	...
	errs := valid.VarWithValue(s, s2, "nefield")
	Equal(t, errs, nil)

	errs = valid.VarWithValue(i2, i, "nefield")
	Equal(t, errs, nil)

	errs = valid.VarWithValue(j2, j, "nefield")
	Equal(t, errs, nil)

	errs = valid.VarWithValue(k2, k, "nefield")
	Equal(t, errs, nil)

	errs = valid.VarWithValue(arr2, arr, "nefield")
	Equal(t, errs, nil)

	errs = valid.VarWithValue(now2, now, "nefield")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "nefield")

	errs = valid.VarWithValue(arr3, arr, "nefield")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "nefield")

	type Test struct {
		Start *time.Time `valid:"nefield=End"`
		End   *time.Time
	}
	...
	errs = valid.VarWithValue(nil, 1, "nefield")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "nefield")

	errs = valid.VarWithValue(sv, now, "nefield")
	Equal(t, errs, nil)

	type Test2 struct {
		Start *time.Time `valid:"nefield=NonExistantField"`
		End   *time.Time
	}
	...
	type Test3 struct {
		Value Other
		Time  time.Time `valid:"nefield=Value"`
	}
...
func TestIsNeValidation(t *testing.T) {
	...
	errs := valid.Var(s, "ne=abcd")
	Equal(t, errs, nil)

	errs = valid.Var(i, "ne=1")
	Equal(t, errs, nil)

	errs = valid.Var(j, "ne=1")
	Equal(t, errs, nil)

	errs = valid.Var(k, "ne=1.543")
	Equal(t, errs, nil)

	errs = valid.Var(arr, "ne=2")
	Equal(t, errs, nil)

	errs = valid.Var(arr, "ne=1")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "ne")

	PanicMatches(t, func() { _ = valid.Var(now, "ne=now") }, "Bad field type time.Time")
}
...
func TestIsEqFieldValidation(t *testing.T) {
	...
	errs := valid.VarWithValue(s, s2, "eqfield")
	Equal(t, errs, nil)

	errs = valid.VarWithValue(i2, i, "eqfield")
	Equal(t, errs, nil)

	errs = valid.VarWithValue(j2, j, "eqfield")
	Equal(t, errs, nil)

	errs = valid.VarWithValue(k2, k, "eqfield")
	Equal(t, errs, nil)

	errs = valid.VarWithValue(arr2, arr, "eqfield")
	Equal(t, errs, nil)

	errs = valid.VarWithValue(now2, now, "eqfield")
	Equal(t, errs, nil)

	errs = valid.VarWithValue(arr3, arr, "eqfield")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "eqfield")

	type Test struct {
		Start *time.Time `valid:"eqfield=End"`
		End   *time.Time
	}
	...
	errs = valid.VarWithValue(nil, 1, "eqfield")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "eqfield")

	channel := make(chan string)
	errs = valid.VarWithValue(5, channel, "eqfield")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "eqfield")

	errs = valid.VarWithValue(5, now, "eqfield")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "eqfield")

	type Test2 struct {
		Start *time.Time `valid:"eqfield=NonExistantField"`
		End   *time.Time
	}
	...
	type TStruct struct {
		Inner     *Inner
		CreatedAt *time.Time `valid:"eqfield=Inner"`
	}
...
func TestIsEqValidation(t *testing.T) {
	...
	errs := valid.Var(s, "eq=abcd")
	Equal(t, errs, nil)

	errs = valid.Var(i, "eq=1")
	Equal(t, errs, nil)

	errs = valid.Var(j, "eq=1")
	Equal(t, errs, nil)

	errs = valid.Var(k, "eq=1.543")
	Equal(t, errs, nil)

	errs = valid.Var(arr, "eq=1")
	Equal(t, errs, nil)

	errs = valid.Var(arr, "eq=2")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "eq")

	PanicMatches(t, func() { _ = valid.Var(now, "eq=now") }, "Bad field type time.Time")
}
...
func TestOneOfValidation(t *testing.T) {
	...
	PanicMatches(t, func() {
		_ = valid.Var(3.14, "oneof=red green")
...
func TestBase64URLValidation(t *testing.T) {
	...
	for _, tc := range testCases {
		err := valid.Var(tc.encoded, "base64url")
		if tc.success {
			Equal(t, err, nil)
...
func TestFileValidation(t *testing.T) {
	...
	for _, test := range tests {
		errs := valid.Var(test.param, "file")
...
func TestEthereumAddressValidation(t *testing.T) {
	...
	for i, test := range tests {
		...
		errs := valid.Var(test.param, "eth_addr")
...
func TestBitcoinAddressValidation(t *testing.T) {
	...
	for i, test := range tests {
		...
		errs := valid.Var(test.param, "btc_addr")
...
func TestBitcoinBech32AddressValidation(t *testing.T) {
	...
	for i, test := range tests {
		...
		errs := valid.Var(test.param, "btc_addr_bech32")
...
func TestNoStructLevelValidation(t *testing.T) {
	type Inner struct {
		Test string `valid:"len=5"`
	}

	type Outer struct {
		InnerStruct *Inner `valid:"required,nostructlevel"`
	}
...
func TestStructOnlyValidation(t *testing.T) {
	...
	type Outer struct {
		InnerStruct *Inner `valid:"required,structonly"`
	}
	...
	type User struct {
		...
		Number         string     `valid:"required,e164"`
		Email          string     `valid:"required,email"`
		FavouriteColor string     `valid:"hexcolor|rgb|rgba"`
		Addresses      []*Address `valid:"required"`   // a person can have a home and cottage...
		Address        Address    `valid:"structonly"` // a person can have a home and cottage...
	}
...
func TestGtField(t *testing.T) {
	...
	type TimeTest struct {
		Start *time.Time `valid:"required,gt"`
		End   *time.Time `valid:"required,gt,gtfield=Start"`
	}
	...
	errs = valid.VarWithValue(&end, &start, "gtfield")
	Equal(t, errs, nil)

	errs = valid.VarWithValue(&start, &end, "gtfield")
	NotEqual(t, errs, nil)
	...
	errs = valid.VarWithValue(&timeTest, &end, "gtfield")
	NotEqual(t, errs, nil)

	errs = valid.VarWithValue("test bigger", "test", "gtfield")
	Equal(t, errs, nil)

	type IntTest struct {
		Val1 int `valid:"required"`
		Val2 int `valid:"required,gtfield=Val1"`
	}
	...
	errs = valid.VarWithValue(int(5), int(1), "gtfield")
	Equal(t, errs, nil)

	errs = valid.VarWithValue(int(1), int(5), "gtfield")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "gtfield")

	type UIntTest struct {
		Val1 uint `valid:"required"`
		Val2 uint `valid:"required,gtfield=Val1"`
	}
	...
	errs = valid.VarWithValue(uint(5), uint(1), "gtfield")
	Equal(t, errs, nil)

	errs = valid.VarWithValue(uint(1), uint(5), "gtfield")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "gtfield")

	type FloatTest struct {
		Val1 float64 `valid:"required"`
		Val2 float64 `valid:"required,gtfield=Val1"`
	}
	...
	errs = valid.VarWithValue(float32(5), float32(1), "gtfield")
	Equal(t, errs, nil)

	errs = valid.VarWithValue(float32(1), float32(5), "gtfield")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "gtfield")

	errs = valid.VarWithValue(nil, 1, "gtfield")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "gtfield")

	errs = valid.VarWithValue(5, "T", "gtfield")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "gtfield")

	errs = valid.VarWithValue(5, start, "gtfield")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "gtfield")

	type TimeTest2 struct {
		Start *time.Time `valid:"required"`
		End   *time.Time `valid:"required,gtfield=NonExistantField"`
	}
	...
	type Test struct {
		Value Other
		Time  time.Time `valid:"gtfield=Value"`
	}
...
func TestLtField(t *testing.T) {
	...
	type TimeTest struct {
		Start *time.Time `valid:"required,lt,ltfield=End"`
		End   *time.Time `valid:"required,lt"`
	}
	...
	errs = valid.VarWithValue(&start, &end, "ltfield")
	Equal(t, errs, nil)

	errs = valid.VarWithValue(&end, &start, "ltfield")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "ltfield")

	errs = valid.VarWithValue(&end, timeTest, "ltfield")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "ltfield")

	errs = valid.VarWithValue("tes", "test", "ltfield")
	Equal(t, errs, nil)

	type IntTest struct {
		Val1 int `valid:"required"`
		Val2 int `valid:"required,ltfield=Val1"`
	}
	...
	errs = valid.VarWithValue(int(1), int(5), "ltfield")
	Equal(t, errs, nil)

	errs = valid.VarWithValue(int(5), int(1), "ltfield")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "ltfield")

	type UIntTest struct {
		Val1 uint `valid:"required"`
		Val2 uint `valid:"required,ltfield=Val1"`
	}
	...
	errs = valid.VarWithValue(uint(1), uint(5), "ltfield")
	Equal(t, errs, nil)

	errs = valid.VarWithValue(uint(5), uint(1), "ltfield")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "ltfield")

	type FloatTest struct {
		Val1 float64 `valid:"required"`
		Val2 float64 `valid:"required,ltfield=Val1"`
	}
	...
	errs = valid.VarWithValue(float32(1), float32(5), "ltfield")
	Equal(t, errs, nil)

	errs = valid.VarWithValue(float32(5), float32(1), "ltfield")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "ltfield")

	errs = valid.VarWithValue(nil, 5, "ltfield")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "ltfield")

	errs = valid.VarWithValue(1, "T", "ltfield")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "ltfield")

	errs = valid.VarWithValue(1, end, "ltfield")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "ltfield")

	type TimeTest2 struct {
		Start *time.Time `valid:"required"`
		End   *time.Time `valid:"required,ltfield=NonExistantField"`
	}
...
func TestFieldContains(t *testing.T) {
	...
	type StringTest struct {
		Foo string `valid:"fieldcontains=Bar"`
		Bar string
	}
	...
	errs = valid.VarWithValue("foo", "bar", "fieldcontains")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "fieldcontains")

	errs = valid.VarWithValue("bar", "foobarfoo", "fieldcontains")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "fieldcontains")

	errs = valid.VarWithValue("foobarfoo", "bar", "fieldcontains")
	Equal(t, errs, nil)
	...
	errs = valid.Struct(stringTestMissingField)
	NotEqual(t, errs, nil)
...
func TestFieldExcludes(t *testing.T) {
	...
	type StringTest struct {
		Foo string `valid:"fieldexcludes=Bar"`
		Bar string
	}
	...
	errs = valid.VarWithValue("foo", "bar", "fieldexcludes")
	Equal(t, errs, nil)

	errs = valid.VarWithValue("bar", "foobarfoo", "fieldexcludes")
	Equal(t, errs, nil)

	errs = valid.VarWithValue("foobarfoo", "bar", "fieldexcludes")
	NotEqual(t, errs, nil)
...
func TestContainsAndExcludes(t *testing.T) {
	...
	type ImpossibleStringTest struct {
		Foo string `valid:"fieldcontains=Bar"`
		Bar string `valid:"fieldexcludes=Foo"`
	}
	...
	errs := valid.Struct(impossibleStringTest)
	NotEqual(t, errs, nil)
...
func TestLteField(t *testing.T) {
	...
	type TimeTest struct {
		Start *time.Time `valid:"required,lte,ltefield=End"`
		End   *time.Time `valid:"required,lte"`
	}
	...
	errs = valid.VarWithValue(&start, &end, "ltefield")
	Equal(t, errs, nil)

	errs = valid.VarWithValue(&end, &start, "ltefield")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "ltefield")

	errs = valid.VarWithValue(&end, timeTest, "ltefield")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "ltefield")

	errs = valid.VarWithValue("tes", "test", "ltefield")
	Equal(t, errs, nil)

	errs = valid.VarWithValue("test", "test", "ltefield")
	Equal(t, errs, nil)

	type IntTest struct {
		Val1 int `valid:"required"`
		Val2 int `valid:"required,ltefield=Val1"`
	}
	...
	errs = valid.VarWithValue(int(1), int(5), "ltefield")
	Equal(t, errs, nil)

	errs = valid.VarWithValue(int(5), int(1), "ltefield")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "ltefield")

	type UIntTest struct {
		Val1 uint `valid:"required"`
		Val2 uint `valid:"required,ltefield=Val1"`
	}
	...
	errs = valid.VarWithValue(uint(1), uint(5), "ltefield")
	Equal(t, errs, nil)

	errs = valid.VarWithValue(uint(5), uint(1), "ltefield")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "ltefield")

	type FloatTest struct {
		Val1 float64 `valid:"required"`
		Val2 float64 `valid:"required,ltefield=Val1"`
	}
	...
	errs = valid.VarWithValue(float32(1), float32(5), "ltefield")
	Equal(t, errs, nil)

	errs = valid.VarWithValue(float32(5), float32(1), "ltefield")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "ltefield")

	errs = valid.VarWithValue(nil, 5, "ltefield")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "ltefield")

	errs = valid.VarWithValue(1, "T", "ltefield")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "ltefield")

	errs = valid.VarWithValue(1, end, "ltefield")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "ltefield")

	type TimeTest2 struct {
		Start *time.Time `valid:"required"`
		End   *time.Time `valid:"required,ltefield=NonExistantField"`
	}
...
func TestGteField(t *testing.T) {
	...
	type TimeTest struct {
		Start *time.Time `valid:"required,gte"`
		End   *time.Time `valid:"required,gte,gtefield=Start"`
	}
	...
	errs = valid.VarWithValue(&end, &start, "gtefield")
	Equal(t, errs, nil)

	errs = valid.VarWithValue(&start, &end, "gtefield")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "gtefield")

	errs = valid.VarWithValue(&start, timeTest, "gtefield")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "gtefield")

	errs = valid.VarWithValue("test", "test", "gtefield")
	Equal(t, errs, nil)

	errs = valid.VarWithValue("test bigger", "test", "gtefield")
	Equal(t, errs, nil)

	type IntTest struct {
		Val1 int `valid:"required"`
		Val2 int `valid:"required,gtefield=Val1"`
	}
	...
	errs = valid.VarWithValue(int(5), int(1), "gtefield")
	Equal(t, errs, nil)

	errs = valid.VarWithValue(int(1), int(5), "gtefield")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "gtefield")

	type UIntTest struct {
		Val1 uint `valid:"required"`
		Val2 uint `valid:"required,gtefield=Val1"`
	}
	...
	errs = valid.VarWithValue(uint(5), uint(1), "gtefield")
	Equal(t, errs, nil)

	errs = valid.VarWithValue(uint(1), uint(5), "gtefield")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "gtefield")

	type FloatTest struct {
		Val1 float64 `valid:"required"`
		Val2 float64 `valid:"required,gtefield=Val1"`
	}
	...
	errs = valid.VarWithValue(float32(5), float32(1), "gtefield")
	Equal(t, errs, nil)

	errs = valid.VarWithValue(float32(1), float32(5), "gtefield")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "gtefield")

	errs = valid.VarWithValue(nil, 1, "gtefield")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "gtefield")

	errs = valid.VarWithValue(5, "T", "gtefield")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "gtefield")

	errs = valid.VarWithValue(5, start, "gtefield")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "gtefield")

	type TimeTest2 struct {
		Start *time.Time `valid:"required"`
		End   *time.Time `valid:"required,gtefield=NonExistantField"`
	}
...
func TestValidateByTagAndValue(t *testing.T) {
	...
	val := "test"
	field := "test"
	errs := valid.VarWithValue(val, field, "required")
	Equal(t, errs, nil)
	...
	errs = valid.RegisterValidation("isequaltestfunc", fn)
	Equal(t, errs, nil)

	errs = valid.VarWithValue(val, field, "isequaltestfunc")
	Equal(t, errs, nil)
...
func TestAddFunctions(t *testing.T) {
	...
	errs := valid.RegisterValidation("new", fn)
	Equal(t, errs, nil)

	errs = valid.RegisterValidation("", fn)
	NotEqual(t, errs, nil)

	errs = valid.RegisterValidation("new", nil)
	NotEqual(t, errs, nil)

	errs = valid.RegisterValidation("new", fn)
	Equal(t, errs, nil)

	errs = valid.RegisterValidationCtx("new", fnCtx)
	Equal(t, errs, nil)

	PanicMatches(t, func() { _ = valid.RegisterValidation("dive", fn) }, "Tag 'dive' either contains restricted characters or is the same as a restricted tag needed for normal operation")
}

func TestChangeTag(t *testing.T) {
	valid := New()
	valid.SetTagName("val")
...
func TestBadParams(t *testing.T) {
	...
	PanicMatches(t, func() { _ = valid.Var(i, "len=a") }, "strconv.ParseInt: parsing \"a\": invalid syntax")
	PanicMatches(t, func() { _ = valid.Var(i, "len=a") }, "strconv.ParseInt: parsing \"a\": invalid syntax")

	var ui uint = 1
	PanicMatches(t, func() { _ = valid.Var(ui, "len=a") }, "strconv.ParseUint: parsing \"a\": invalid syntax")

	f := 1.23
	PanicMatches(t, func() { _ = valid.Var(f, "len=a") }, "strconv.ParseFloat: parsing \"a\": invalid syntax")
}

func TestLength(t *testing.T) {
	valid := New()
	i := true
	PanicMatches(t, func() { _ = valid.Var(i, "len") }, "Bad field type bool")
}

func TestIsGt(t *testing.T) {
	valid := New()
	myMap := map[string]string{}
	errs := valid.Var(myMap, "gt=0")
	NotEqual(t, errs, nil)

	f := 1.23
	errs = valid.Var(f, "gt=5")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "gt")

	var ui uint = 5
	errs = valid.Var(ui, "gt=10")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "gt")

	i := true
	PanicMatches(t, func() { _ = valid.Var(i, "gt") }, "Bad field type bool")

	tm := time.Now().UTC()
	tm = tm.Add(time.Hour * 24)

	errs = valid.Var(tm, "gt")
	Equal(t, errs, nil)

	t2 := time.Now().UTC().Add(-time.Hour)

	errs = valid.Var(t2, "gt")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "gt")

	type Test struct {
		Now *time.Time `valid:"gt"`
	}
	s := &Test{
		Now: &tm,
	}
...
func TestIsGte(t *testing.T) {
	valid := New()
	i := true
	PanicMatches(t, func() { _ = valid.Var(i, "gte") }, "Bad field type bool")

	t1 := time.Now().UTC()
	t1 = t1.Add(time.Hour * 24)

	errs := valid.Var(t1, "gte")
	Equal(t, errs, nil)

	t2 := time.Now().UTC().Add(-time.Hour)

	errs = valid.Var(t2, "gte")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "gte")

	type Test struct {
		Now *time.Time `valid:"gte"`
	}
	s := &Test{
		Now: &t1,
	}
...
func TestIsLt(t *testing.T) {
	valid := New()
	myMap := map[string]string{}
	errs := valid.Var(myMap, "lt=0")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "lt")

	f := 1.23
	errs = valid.Var(f, "lt=0")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "lt")

	var ui uint = 5
	errs = valid.Var(ui, "lt=0")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "lt")

	i := true
	PanicMatches(t, func() { _ = valid.Var(i, "lt") }, "Bad field type bool")

	t1 := time.Now().UTC().Add(-time.Hour)

	errs = valid.Var(t1, "lt")
	Equal(t, errs, nil)

	t2 := time.Now().UTC()
	t2 = t2.Add(time.Hour * 24)

	errs = valid.Var(t2, "lt")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "lt")

	type Test struct {
		Now *time.Time `valid:"lt"`
	}
...
func TestIsLte(t *testing.T) {
	...
	i := true
	PanicMatches(t, func() { _ = valid.Var(i, "lte") }, "Bad field type bool")

	t1 := time.Now().UTC().Add(-time.Hour)

	errs := valid.Var(t1, "lte")
	Equal(t, errs, nil)

	t2 := time.Now().UTC()
	t2 = t2.Add(time.Hour * 24)

	errs = valid.Var(t2, "lte")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "lte")

	type Test struct {
		Now *time.Time `valid:"lte"`
	}
...
func TestUrnRFC2141(t *testing.T) {
	...
	i := 1
	PanicMatches(t, func() { _ = valid.Var(i, tag) }, "Bad field type int")
}
...
func TestUrl(t *testing.T) {
	...
	i := 1
	PanicMatches(t, func() { _ = valid.Var(i, "url") }, "Bad field type int")
}
...
func TestUri(t *testing.T) {
	...
	i := 1
	PanicMatches(t, func() { _ = valid.Var(i, "uri") }, "Bad field type int")
}
...
func TestOrTag(t *testing.T) {
	...
	s := "rgba(0,31,255,0.5)"
	errs := valid.Var(s, "rgb|rgba")
	Equal(t, errs, nil)

	s = "rgba(0,31,255,0.5)"
	errs = valid.Var(s, "rgb|rgba|len=18")
	Equal(t, errs, nil)
	...
	s = "this ain't right"
	errs = valid.Var(s, "rgb|rgba|len=10")
	NotEqual(t, errs, nil)
	AssertError(t, errs, "", "", "", "", "rgb|rgba|len=10")

	s = "this is right"
	errs = valid.Var(s, "rgb|rgba|len=13")
	Equal(t, errs, nil)

	s = ""
	errs = valid.Var(s, "omitempty,rgb|rgba")
	Equal(t, errs, nil)

	s = "green"
	errs = valid.Var(s, "eq=|eq=blue,rgb|rgba") // should fail on first validation block
	NotEqual(t, errs, nil)
	ve := errs.(ValidationErrors)
	Equal(t, len(ve), 1)
	...
	PanicMatches(t, func() { _ = valid.Var(s, "rgb||len=13") }, "Invalid validation tag on field ''")
	PanicMatches(t, func() { _ = valid.Var(s, "rgb|rgbaa|len=13") }, "Undefined validation function 'rgbaa' on field ''")
	...
	type Colors struct {
		Fav string `valid:"rgb|rgba" json:"fc"`
	}
...
func TestStructInt32Validation(t *testing.T) {
	type TestInt32 struct {
		...
		MinMax    int `valid:"min=1,max=10"`
		Lt        int `valid:"lt=10"`
		Lte       int `valid:"lte=10"`
		Gt        int `valid:"gt=10"`
		Gte       int `valid:"gte=10"`
		OmitEmpty int `valid:"omitempty,min=1,max=10"`
	}
...
func TestInvalidStruct(t *testing.T) {
	...
	err := valid.Struct(s.Test)
	NotEqual(t, err, nil)
	Equal(t, err.Error(), "validator: (nil string)")

	err = valid.Struct(nil)
	NotEqual(t, err, nil)
	Equal(t, err.Error(), "validator: (nil)")

	err = valid.StructPartial(nil, "SubTest.Test")
	NotEqual(t, err, nil)
	Equal(t, err.Error(), "validator: (nil)")

	err = valid.StructExcept(nil, "SubTest.Test")
	NotEqual(t, err, nil)
	Equal(t, err.Error(), "validator: (nil)")
}
...
func TestInvalidValidatorFunction(t *testing.T) {
	...
	PanicMatches(t, func() { _ = valid.Var(s.Test, "zzxxBadFunction") }, "Undefined validation function 'zzxxBadFunction' on field ''")
}
...
func TestCustomFieldName(t *testing.T) {
	...
	type A struct {
		B string `schema:"b" valid:"required"`
		C string `schema:"c" valid:"required"`
		D []bool `schema:"d" valid:"required"`
		E string `schema:"-" valid:"required"`
	}
...
func TestMutipleRecursiveExtractStructCache(t *testing.T) {
	...
	type Recursive struct {
		Field *string `valid:"required,len=5,ne=string"`
	}
	...
	sc := valid.extractStructCache(current, name)
...
// Thanks @robbrockbank, see https://github.com/bingoohuang/valid/issues/249
func TestPointerAndOmitEmpty(t *testing.T) {
	valid := New()

	type Test struct {
		MyInt *int `valid:"omitempty,gte=2,lte=255"`
	}
...
func TestRequired(t *testing.T) {
	...
	type Test struct {
		Value interface{} `valid:"required"`
	}
...
func TestTranslations(t *testing.T) {
	...
	valid := New()
	err := valid.RegisterTranslation("required", trans,
	...
	err = valid.RegisterTranslation("required", fr,
		func(ut ut.Translator) (err error) {
			// using this stype because multiple translation may have to be added for the full translation
...
func TestStructFiltered(t *testing.T) {
	...
	// the following should all return no errors as everything is valid in
	// the default state
	errs := valid.StructFilteredCtx(context.Background(), tPartial, p1)
	Equal(t, errs, nil)

	errs = valid.StructFiltered(tPartial, p2)
	Equal(t, errs, nil)

	// this isn't really a robust test, but is ment to illustrate the ANON CASE below
	errs = valid.StructFiltered(tPartial.SubSlice[0], p3)
	Equal(t, errs, nil)
	...
	// inversion and retesting Partial to generate failures:
	errs = valid.StructFiltered(tPartial, p1)
	NotEqual(t, errs, nil)
	AssertError(t, errs, "TestPartial.Required", "TestPartial.Required", "Required", "Required", "required")
	...
	dt := time.Now()
	err := valid.StructFiltered(&dt, func(ns []byte) bool { return true })
	NotEqual(t, err, nil)
...
func TestRequiredPtr(t *testing.T) {
	...
	type Test3 struct {
		Arr []string `valid:"required"`
	}

	var test3 Test3

	err = valid.Struct(test3)
	NotEqual(t, err, nil)
	...
	type Test4 struct {
		Arr *[]string `valid:"required"` // I know I know pointer to array, just making sure validation works as expected...
	}

	var test4 Test4

	err = valid.Struct(test4)
	NotEqual(t, err, nil)
...
func TestAlphaUnicodeValidation(t *testing.T) {
	...
	for i, test := range tests {
		...
		errs := valid.Var(test.param, "alphaunicode")
...
func TestAlphanumericUnicodeValidation(t *testing.T) {
	...
	for i, test := range tests {
		...
		errs := valid.Var(test.param, "alphanumunicode")
...
func TestArrayStructNamespace(t *testing.T) {
	...
	var input struct {
		Children []child `json:"children" valid:"required,gt=0,dive"`
	}
	input.Children = []child{{"ok"}, {""}}

	errs := valid.Struct(input)
	NotEqual(t, errs, nil)
...
func TestMapStructNamespace(t *testing.T) {
	...
	var input struct {
		Children map[int]child `json:"children" valid:"required,gt=0,dive"`
	}
...
func TestFieldLevelName(t *testing.T) {
	type Test struct {
		String string            `valid:"custom1"      json:"json1"`
		Array  []string          `valid:"dive,custom2" json:"json2"`
		Map    map[string]string `valid:"dive,custom3" json:"json3"`
		Array2 []string          `valid:"custom4"      json:"json4"`
		Map2   map[string]string `valid:"custom5"      json:"json5"`
	}
	...
	err := valid.RegisterValidation("custom1", func(fl FieldLevel) bool {
		res1 = fl.FieldName()
	...
	err = valid.RegisterValidation("custom2", func(fl FieldLevel) bool {
		res2 = fl.FieldName()
	...
	err = valid.RegisterValidation("custom3", func(fl FieldLevel) bool {
		res3 = fl.FieldName()
	...
	err = valid.RegisterValidation("custom4", func(fl FieldLevel) bool {
		res4 = fl.FieldName()
	...
	err = valid.RegisterValidation("custom5", func(fl FieldLevel) bool {
		res5 = fl.FieldName()
...
func TestValidateStructRegisterCtx(t *testing.T) {
	...
	type Test struct {
		Field string `valid:"val"`
	}

	var tst Test

	valid := New()
	err := valid.RegisterValidationCtx("val", fnCtx)
	Equal(t, err, nil)

	valid.RegisterStructValidationCtx(slFn, Test{})

	ctx := context.WithValue(context.Background(), &ctxVal, "testval")
	ctx = context.WithValue(ctx, &ctxSlVal, "slVal")
	errs := valid.StructCtx(ctx, tst)
	Equal(t, errs, nil)
...
func TestHostnameRFC952Validation(t *testing.T) {
	...
	for i, test := range tests {
		...
		errs := valid.Var(test.param, "hostname")
...
func TestHostnameRFC1123Validation(t *testing.T) {
	...
	for i, test := range tests {
		...
		errs := valid.Var(test.param, "hostname_rfc1123")
...
func TestHostnameRFC1123AliasValidation(t *testing.T) {
	...
	valid := New()
	valid.RegisterAlias("hostname", "hostname_rfc1123")
...
func TestFQDNValidation(t *testing.T) {
	...
	for i, test := range tests {
		...
		errs := valid.Var(test.param, "fqdn")
...
func TestIsDefault(t *testing.T) {
	...
	type Inner struct {
		String string `valid:"isdefault"`
	}
	type Test struct {
		String string `valid:"isdefault"`
		Inner  *Inner `valid:"isdefault"`
	}

	var tt Test

	errs := valid.Struct(tt)
	Equal(t, errs, nil)
	...
	type Test2 struct {
		Inner Inner2 `valid:"isdefault" json:"inner"`
	}
...
func TestUniqueValidation(t *testing.T) {
	...
	for i, test := range tests {
		...
		errs := valid.Var(test.param, "unique")
	...
	PanicMatches(t, func() { _ = valid.Var(1.0, "unique") }, "Bad field type float64")
}
...
func TestUniqueValidationStructSlice(t *testing.T) {
	...
	for i, test := range tests {
		...
		errs := valid.Var(test.target, test.param)
	...
	PanicMatches(t, func() { valid.Var(testStructs, "unique=C") }, "Bad field name C")
}
...
func TestHTMLValidation(t *testing.T) {
	...
	for i, test := range tests {
		...
		errs := valid.Var(test.param, "html")
...
func TestHTMLEncodedValidation(t *testing.T) {
	...
	for i, test := range tests {
		...
		errs := valid.Var(test.param, "html_encoded")
...
func TestURLEncodedValidation(t *testing.T) {
	...
	for i, test := range tests {
		...
		errs := valid.Var(test.param, "url_encoded")
...
func TestKeys(t *testing.T) {
	type Test struct {
		Test1 map[string]string `valid:"gt=0,dive,keys,eq=testkey,endkeys,eq=testval" json:"test1"`
		Test2 map[int]int       `valid:"gt=0,dive,keys,eq=3,endkeys,eq=4"             json:"test2"`
		Test3 map[int]int       `valid:"gt=0,dive,keys,eq=3,endkeys"                  json:"test3"`
	}
	...
	type Test2 struct {
		NestedKeys map[[1]string]string `valid:"gt=0,dive,keys,dive,eq=innertestkey,endkeys,eq=outertestval"`
	}

	var tst2 Test2

	err = valid.Struct(tst2)
	NotEqual(t, err, nil)
	...
	PanicMatches(t, func() { _ = valid.Var(map[string]string{"key": "val"}, "endkeys,dive,eq=val") }, "'endkeys' tag encountered without a corresponding 'keys' tag")
	PanicMatches(t, func() { _ = valid.Var(1, "keys,eq=1,endkeys") }, "'keys' tag must be immediately preceded by the 'dive' tag")
...
func TestKeysCustomValidation(t *testing.T) {
	...
	type TestMapStructPtr struct {
		Label Label `valid:"dive,keys,lang_code,endkeys,required"`
	}

	valid := New()
	err := valid.RegisterValidation("lang_code", func(fl FieldLevel) bool {
	...
	err = valid.Struct(TestMapStructPtr{label})
	NotEqual(t, err, nil)
...
func TestKeyOrs(t *testing.T) {
	type Test struct {
		Test1 map[string]string `valid:"gt=0,dive,keys,eq=testkey|eq=testkeyok,endkeys,eq=testval" json:"test1"`
	}
	...
	valid.RegisterAlias("okkey", "eq=testkey|eq=testkeyok")

	type Test2 struct {
		Test1 map[string]string `valid:"gt=0,dive,keys,okkey,endkeys,eq=testval" json:"test1"`
	}
...
func TestStartsWithValidation(t *testing.T) {
	tests := []struct {
		Value       string `valid:"startswith=(/^ヮ^)/*:・ﾟ✧"`
		Tag         string
		ExpectedNil bool
	}{
...
func TestEndsWithValidation(t *testing.T) {
	tests := []struct {
		Value       string `valid:"endswith=(/^ヮ^)/*:・ﾟ✧"`
		Tag         string
		ExpectedNil bool
	}{
...
func TestRequiredWith(t *testing.T) {
	...
	test := struct {
		Inner   *Inner
		FieldE  string            `valid:"omitempty" json:"field_e"`
		FieldER string            `valid:"required_with=FieldE" json:"field_er"`
		Field1  string            `valid:"omitempty" json:"field_1"`
		Field2  *string           `valid:"required_with=Field1" json:"field_2"`
		Field3  map[string]string `valid:"required_with=Field2" json:"field_3"`
		Field4  interface{}       `valid:"required_with=Field3" json:"field_4"`
		Field5  string            `valid:"required_with=Inner.Field" json:"field_5"`
	}{
	...
	test2 := struct {
		...
		Field2  *string           `valid:"required_with=Field1" json:"field_2"`
		Field3  map[string]string `valid:"required_with=Field2" json:"field_3"`
		Field4  interface{}       `valid:"required_with=Field2" json:"field_4"`
		Field5  string            `valid:"required_with=Field3" json:"field_5"`
		Field6  string            `valid:"required_with=Inner.Field" json:"field_6"`
		Field7  string            `valid:"required_with=Inner2.Field" json:"field_7"`
	}{
		Inner:  &Inner{Field: &fieldVal},
		Field2: &fieldVal,
	}
...
func TestRequiredWithAll(t *testing.T) {
	...
	test := struct {
		Inner   *Inner
		FieldE  string            `valid:"omitempty" json:"field_e"`
		FieldER string            `valid:"required_with_all=FieldE" json:"field_er"`
		Field1  string            `valid:"omitempty" json:"field_1"`
		Field2  *string           `valid:"required_with_all=Field1" json:"field_2"`
		Field3  map[string]string `valid:"required_with_all=Field2" json:"field_3"`
		Field4  interface{}       `valid:"required_with_all=Field3" json:"field_4"`
		Field5  string            `valid:"required_with_all=Inner.Field" json:"field_5"`
	}{
		Inner:  &Inner{Field: &fieldVal},
		Field1: "test_field1",
		Field2: &fieldVal,
	...
	test2 := struct {
		...
		Field2  *string           `valid:"required_with_all=Field1" json:"field_2"`
		Field3  map[string]string `valid:"required_with_all=Field2" json:"field_3"`
		Field4  interface{}       `valid:"required_with_all=Field1 FieldE" json:"field_4"`
		Field5  string            `valid:"required_with_all=Inner.Field Field2" json:"field_5"`
		Field6  string            `valid:"required_with_all=Inner2.Field Field2" json:"field_6"`
	}{
		Inner:  &Inner{Field: &fieldVal},
		Field2: &fieldVal,
	}
...
func TestRequiredWithout(t *testing.T) {
	...
	test := struct {
		Inner  *Inner
		Field1 string            `valid:"omitempty" json:"field_1"`
		Field2 *string           `valid:"required_without=Field1" json:"field_2"`
		Field3 map[string]string `valid:"required_without=Field2" json:"field_3"`
		Field4 interface{}       `valid:"required_without=Field3" json:"field_4"`
		Field5 string            `valid:"required_without=Field3" json:"field_5"`
	}{
		Inner:  &Inner{Field: &fieldVal},
		Field2: &fieldVal,
		Field3: map[string]string{"key": "val"},
	...
	test2 := struct {
		...
		Field4 interface{}       `valid:"required_without=Field3" json:"field_4"`
		Field5 string            `valid:"required_without=Field3" json:"field_5"`
		Field6 string            `valid:"required_without=Field1" json:"field_6"`
		Field7 string            `valid:"required_without=Inner.Field" json:"field_7"`
		Field8 string            `valid:"required_without=Inner.Field" json:"field_8"`
	}{
		Inner:  &Inner{},
		Field3: map[string]string{"key": "val"},
		Field4: "test",
...
func TestRequiredWithoutAll(t *testing.T) {
	...
	test := struct {
		Field1 string            `valid:"omitempty" json:"field_1"`
		Field2 *string           `valid:"required_without_all=Field1" json:"field_2"`
		Field3 map[string]string `valid:"required_without_all=Field2" json:"field_3"`
		Field4 interface{}       `valid:"required_without_all=Field3" json:"field_4"`
		Field5 string            `valid:"required_without_all=Field3" json:"field_5"`
	}{
		Field1: "",
		Field2: &fieldVal,
		Field3: map[string]string{"key": "val"},
	...
	test2 := struct {
		...
		Field4 interface{}       `valid:"required_without_all=Field3" json:"field_4"`
		Field5 string            `valid:"required_without_all=Field3" json:"field_5"`
		Field6 string            `valid:"required_without_all=Field1 Field3" json:"field_6"`
	}{
		Field3: map[string]string{"key": "val"},
		Field4: "test",
		Field5: "test",
...
func TestLookup(t *testing.T) {
	type Lookup struct {
		FieldA *string `json:"fieldA,omitempty" valid:"required_without=FieldB"`
		FieldB *string `json:"fieldB,omitempty" valid:"required_without=FieldA"`
	}
...
func TestRequiredWithoutPointers(t *testing.T) {
	type Lookup struct {
		FieldA *bool `json:"fieldA,omitempty" valid:"required_without=FieldB"`
		FieldB *bool `json:"fieldB,omitempty" valid:"required_without=FieldA"`
	}
...
func TestRequiredWithoutAllPointers(t *testing.T) {
	type Lookup struct {
		FieldA *bool `json:"fieldA,omitempty" valid:"required_without_all=FieldB"`
		FieldB *bool `json:"fieldB,omitempty" valid:"required_without_all=FieldA"`
	}
...
func TestJSONValidation(t *testing.T) {
	...
	for i, test := range tests {
		...
		errs := valid.Var(test.param, "json")
...
func Test_hostnameport_validator(t *testing.T) {
	type Host struct {
		Addr string `valid:"hostname_port"`
	}
...
func TestLowercaseValidation(t *testing.T) {
	...
	for i, test := range tests {
		...
		errs := valid.Var(test.param, "lowercase")
...
func TestUppercaseValidation(t *testing.T) {
	...
	for i, test := range tests {
		...
		errs := valid.Var(test.param, "uppercase")
	...
	PanicMatches(t, func() {
		_ = valid.Var(2, "uppercase")
	}, "Bad field type int")
}

func TestDatetimeValidation(t *testing.T) {
	tests := []struct {
		value    string `valid:"datetime=2006-01-02"`
		tag      string
		expected bool
	}{
	...
	for i, test := range tests {
		...
		errs := valid.Var(test.value, test.tag)
