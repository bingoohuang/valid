/*
Package valid implements value validations for structs and individual fields
based on tags.

It can also handle Cross-Field and Cross-Struct validation for nested structs
and has the ability to dive into arrays and maps of any type.

see more examples https://github.com/bingoohuang/valid/tree/master/_examples
...
where err is always != nil:
	...
	https://github.com/bingoohuang/valid/issues/134
...
it to type ValidationErrors like so err.(valid.ValidationErrors).
...
Custom Validation functions can be added. Example:
	...
	// Structure
	func customFunc(fl valid.FieldLevel) bool {
	...
	valid.RegisterValidation("custom tag name", customFunc)
...
used "eqcsfield" it could be multiple levels down. Example:
	...
	type Outer struct {
		...
		CreatedAt time.Time      `valid:"ltecsfield=InnerStructField.StartDate"`
	}
	...
	// NOTE: when calling valid.Struct(val) topStruct will be the top level struct passed
	//       into the function
	//       when calling valid.VarWithValue(val, field, tag) val will be
	//       whatever you pass, struct, field...
	//       when calling valid.Field(field, tag) val will be nil
...
so the above will become excludesall=0x2C.
	...
	type Test struct {
		Field `valid:"excludesall=,"`    // BAD! Do not include a comma.
		Field `valid:"excludesall=0x2C"` // GOOD! Use the UTF-8 hex representation.
	}
...
so the above will become excludesall=0x7C
	...
	type Test struct {
		Field `valid:"excludesall=|"`    // BAD! Do not include a a pipe!
		Field `valid:"excludesall=0x7C"` // GOOD! Use the UTF-8 hex representation.
	}
...
This tells the validator to dive into a slice, array or map and valid that
...
Multidimensional nesting is also supported, each level you wish to valid will
...
This will valid the field value against another fields value either within
...
Example #2:
	...
	// Validating by field:
	valid.VarWithValue(password, confirmpassword, "eqfield")
...
Examples:
	...
	// Validating by field:
	valid.VarWithValue(color1, color2, "nefield")
...
Only valid for Numbers and time.Time types, this will valid the field value
...
Example #1:
	...
	valid.Struct Usage(gtfield=Start)

Example #2:

	// Validating by field:
	valid.VarWithValue(start, end, "gtfield")
...
Example #1:
	...
	// Validation on End field using:
	valid.Struct Usage(gtefield=Start)

Example #2:

	// Validating by field:
	valid.VarWithValue(start, end, "gtefield")
...
Example #1:
	...
	// Validation on End field using:
	valid.Struct Usage(ltfield=Start)

Example #2:

	// Validating by field:
	valid.VarWithValue(start, end, "ltfield")
...
Example #1:
	...
	// Validation on End field using:
	valid.Struct Usage(ltefield=Start)

Example #2:

	// Validating by field:
	valid.VarWithValue(start, end, "ltefield")
...
NOTE: this will also valid that the data portion is valid base64
...
Example of registration and use:
	...
	type Test struct {
		TestField string `valid:"yourtag"`
	}
	...
	valid := valid.New()
	valid.RegisterValidation("yourtag", validators.NotBlank)
...
that should not make it to production.
	...
	type Test struct {
		TestField string `valid:"nonexistantfunction=1"`
	}
	...
	valid.Struct(t) // this will panic
*/
package valid
